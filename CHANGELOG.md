...
- **Physics-asset diagnosis + authoring pack (`animation`) — 7 new actions, 2 extended.** Ragdoll problems could previously only be diagnosed and fixed in the Physics Asset editor UI: the read surface reported primitive *counts* with no dimensions, no transforms and no constraint frames, there was no way to move a capsule, no way to reach `CollisionDisableTable` (a plain `TMap`, invisible to reflection and therefore to Python), and no way to add or delete a body.
  - `analyze_physics_asset` — run 12 diagnostic checks and return ranked, actionable findings. Params: `asset_path`, `symptom` (`launch_on_contact` / `explodes_immediately` / `sinks_through_floor` / `jitters` / `none`), `overlap_tolerance`, `include_matrix`, `max_findings`, `checks`. Checks cover asset integrity (C0 gate), degenerate geometry, constraint-frame orthonormality, ref-pose joint pre-load, ref-pose limit violations, the body-overlap matrix vs the collision table, collision-state disagreement, mass ratios, CoM lever arms, kinematic pairing, projection energy risk, and solver/profile posture. Returns `analysis_reliable`, `blockers`, `summary`, `findings` (each with `severity`, `contact_gated`, `measured`, `threshold`, `ratio` and a machine-usable `suggested_fix`), `ranked_causes`, `next_steps` and a `data` block with the overlap matrix and per-body masses. Read-only, so it is safe to run before and after every fix. Ranking is symptom-aware: a contact-gated symptom promotes the checks that are dormant in free-fall, which is what separates "the floor launches it" from "it was born violated".
  - `set_physics_body_geometry` — write, add, or remove one collision primitive. Params: `asset_path`, `bone_name`, `primitive_index`, `operation` (`modify`/`add`/`remove`), `shape_type`, `center`, `rotation`, `radius`, `length`, `extent_x`/`_y`/`_z`, `radius_0`/`radius_1`, `scale`, `rest_offset`, `contributes_to_mass`, `collision_enabled`, `name`. Returns `before`/`after` primitive records, `computed_mass_kg_before`/`_after` (resizing a capsule silently rewrites its mass) and `warnings` for engine clamps and sub-`MinPrimSize` dimensions. Box `extent_*` are FULL extents; capsule `length` is the line-segment length.
  - `snap_constraint_to_bone` — PhAT's "Snap Constraint to Bone", via `FConstraintInstance::SnapTransformsToDefault`. Params: `asset_path`, optional locator (omit to snap every constraint), `components`, `dry_run`. Returns per-constraint `frame1_pos_delta_cm` / `frame1_rot_delta_deg` / `frame2_*` and `changed_count`. Hard-errors before touching anything when the asset has no preview mesh (the default parent transform would silently collapse to Identity) or when a targeted constraint names a bone absent from the ref skeleton (`CalculateRelativeBoneTransform` would assert and crash the editor).
  - `set_physics_constraint_frames` — explicitly author a constraint's reference frames. Params: `asset_path`, locator, `frame1_position`/`_pri_axis`/`_sec_axis`/`_rotation`, the `frame2_*` equivalents, `angular_rotation_offset`. Axes are normalized and Gram-Schmidt'd server-side, so this action cannot author the non-orthonormal basis check C2 flags. Returns `before`/`after` plus `ref_pose_linear_error_cm_before`/`_after` and the angular equivalents.
  - `set_physics_collision_pairs` — read/write `UPhysicsAsset::CollisionDisableTable`. Params: `asset_path`, `pairs` (`[{bone_1, bone_2, enabled}]`), `disable_overlapping`, `overlap_tolerance`, `include_adjacent`, `dry_run`. `disable_overlapping` re-runs the engine's generation-time overlap pass analytically (segment/segment, no temp world), which is the fix for the table auto-generation built once and never recomputed after the capsules were edited. Returns `changed`, `table_size_before`/`_after` and `still_colliding` — empty is the success condition.
  - `add_physics_body` — create a body on a bone that has none, through `FPhysicsAssetUtils::CreateNewBody` so the index map, bounds bodies and collision table stay consistent. Params: `asset_path`, `bone_name`, `shape_type`, `auto_size`, explicit `radius`/`length`/`extent_*`, `physics_type`, `create_constraint`, `disable_collision_with_parent`, `disable_collision_with_all`. Seeds a capsule along the bone → nearest-child axis and creates + snaps the constraint to the nearest body-bearing ancestor.
  - `remove_physics_body` — delete a body through `FPhysicsAssetUtils::DestroyBody`, so the index-keyed collision table is re-indexed and attached constraints are destroyed rather than orphaned. Params: `asset_path`, `bone_name` or `body_index`. Returns `removed_constraints` and `orphaned_bodies` (bodies left with no path to the root through the constraint graph — a mid-chain delete silently disconnects everything below it).
  - `get_physics_asset_info` **extended** (non-breaking) with `bone_name` / `constraint_index` filters and `include_geometry` / `include_frames` / `include_solver` / `include_collision_table` / `include_derived`. Bodies gain per-primitive dimensions, transforms, authored-vs-effective sizes, volume and density; constraints gain both reference frames, the frame determinant error, linear DOFs, projection / shock-propagation / mass-conditioning, breakables and `pair_collision_enabled`; the asset gains solver type + settings, total mass and the collision-disable table.
  - `set_constraint_properties` **extended** (non-breaking) with the linear DOFs (`linear_x_motion`/`_y_`/`_z_`, `linear_limit`), the projection block (`enable_projection`, `projection_linear_tolerance`/`_angular_tolerance`/`_linear_alpha`/`_angular_alpha`), `enable_shock_propagation` + `shock_propagation_alpha`, `parent_dominates`, `enable_mass_conditioning`, `contact_transfer_scale`, soft cone/twist limits, and `apply_to_all`.
  - All eight mutators are wired into `animation.batch_execute` (per-body resize and per-constraint edits are exactly its workload), and none of them save the package — run `editor.list_dirty_packages` then `editor.save_packages` afterwards.
### Fixed

- **`get_physics_asset_info` reported a meaningless `mass` (`animation`).** The field was `FBodyInstance::GetMassOverride()`, which returns the struct default (100) on every body whose `bOverrideMass` is false — so any mass-ratio analysis on the old output computed a ratio of exactly 1.0 for every pair and concluded "no mass problems". Bodies now report `computed_mass_kg` from `UBodySetup::CalculateMass(nullptr)` alongside `mass_override_kg`, `override_mass`, `mass_scale`, `volume_cm3` and `density`.

## [0.22.0] - 2026-08-01

### Internal

Three release gates that reported green while the path they defend was open. None of these change the plugin at runtime; all three affect what can reach a release.

- **The offline CLI build gate now actually fails.** `Tools/MonolithQuery/build.bat` wrapped its compile in a parenthesised `if %ERRORLEVEL% equ 0 (...)` block, and `cmd.exe` expands `%VAR%` for an entire block when the block is *parsed* — so the inner check read the exit code of the preceding `where cl`, not the compiler's. The test was literally `if 0 equ 0`, its failure branch was unreachable, and a failed compile fell through to the copy step, published a **stale** `monolith_query.exe`, printed success and exited 0. Gating now uses conditional execution (`||`), which reads each command's real exit code and fires on any non-zero — including the negative codes a compiler hard fault returns, which the obvious `if not errorlevel 1` fix would still have treated as success, since that test is a signed `>=` comparison. The copy step is gated too; it was previously unchecked.
- **The offline-exe freshness guard is wired in.** `Scripts/check_offline_exe_fresh.py` was documented in `SPEC_CORE.md` and `API_REFERENCE.md` as a guard but was invoked by no script. It now runs in `make_release.ps1` immediately after the CLI is staged and before the parity guard, where a failure aborts with no artifact written. Its scope is now stated in both documents: it hashes `monolith_query.cpp` only, so a failure originating in `ThirdParty/sqlite3.c`, a ThirdParty header, or the toolchain is covered by `build.bat`'s exit code rather than by this guard.
- **The pre-v2 SHA marker detector now matches the client's own semantics.** `Scripts/verify_release_body.ps1` searched for pre-v2 markers with a line-anchored pattern tolerating only leading whitespace, while the deployed parser it defends uses an *unanchored* regex whose `\s*` spans newlines. The gate was therefore weaker than the thing it protects: an ordinary markdown bullet, a table cell, any prose prefix, or a hash on the following line passed the gate and would still hard-crash every deployed v0.14.7-v0.21.0 Windows client that clicks Install (issues #90/#94). The detector is now unanchored, which cannot false-positive on the `-v2` names, and a `-SelfTest` switch asserts all four escape shapes are rejected and valid v2 bodies accepted — so the regex cannot be quietly re-anchored later.

Two further internal changes, neither of them a release gate:

- Added `Source/MonolithIndex/Private/Tests/` with `MonolithProjectSearchTests.cpp` — four automation tests over a self-contained temp index (no project index, no editor state): column-qualified search returns results, a genuine syntax error and an unknown column are caller errors, a zero-match query is still a success, a closed database is an internal error, `limit` is clamped, and the action's parameter validation returns `-32602`.
- The offline `monolith_query.exe` and `monolith_offline.py` mirror the live search classification, the `limit` clamp and the query-length cap. `monolith_query.cpp` gains a `query_checked` helper that reports SQLite failures instead of logging to stderr and returning an empty row set; `monolith_offline.py` widens its handler from `sqlite3.OperationalError` to `sqlite3.DatabaseError`, so a corrupt index is reported as a structured error rather than escaping as a traceback.
...
- **`project get_stats` reports assets skipped by deep indexing** — `skipped_assets` (count) and `skipped_asset_paths` (up to 50 paths). Populated only when the index dropped an asset after repeated interrupted attempts; `monolith_reindex force=true` clears it.
- **`Monolith.StartIndex force`** — the console command now takes an optional `force` argument. Bare `Monolith.StartIndex` resumes an interrupted index; `force` wipes and starts over.

### Fixed

- **`animation set_anim_graph_node_property` no longer writes to a different node when its scope fails to resolve.** `graph_name` / `state_name` were advisory: an unresolved scope fell through to the all-graphs search by `node_id`, and node ids are not unique across graphs — sibling states routinely hold identical ones — so the write landed in an unrelated animation layer. Nothing downstream catches this: the asset still compiles and still validates, it just plays the wrong animation. A **supplied** scope is now authoritative and errors when it does not resolve to exactly one graph, including when it matches more than one (previously the first match won). Omitting both parameters still searches all graphs, which is the documented default and unchanged. Note the one tightening: `graph_name: "AnimGraph"` used to be ignored and now scopes for real, so a node outside the main AnimGraph is reported as not found instead of being resolved elsewhere.
- **`animation set_anim_graph_node_property` can address nested state machines.** The scope resolver walked function graphs plus a single level of state machine, so a grandchild state — `Standing States → Stop → Stop States → Plant Left Foot` — was unreachable by any spelling of `graph_name` / `state_name`. It now enumerates with `UBlueprint::GetAllGraphs`, which recurses sub-graphs, matching the enumeration `set_anim_node_pin_binding` already used to reach the same graphs. `state_name` resolves a state's inner graph at any depth, and pairing it with `graph_name` (the owning state machine) disambiguates sibling states.
- **Enum widget variables compile to enum properties instead of `int`.** `ui add_widget_variable` built `enum:<Name>` pin types with `PinCategory = PC_Enum`. `PC_Enum` is a type-*picker* category and is never a valid pin category: the editor's own type tree rewrites it to `PC_Byte` the instant it builds a pin type, and `FKismetCompilerUtilities::CreatePropertyOnScope` has no `PC_Enum` branch at all — so the variable compiled through the generic `FIntProperty` fallback, and every Get/Set node on it materialised plain `int` pins that refuse a real enum connection. The tell is a "not compatible" message naming a *spaced* enum (`ESlate Visibility Enum`). Enum pins are now built the one correct way, `PC_Byte` plus the `UEnum` as the sub-category object, which is right for a native `enum class : uint8` (compiles to `FEnumProperty`), a `UUserDefinedEnum` asset, and a `TEnumAsByte`-style enum alike. The `blueprint` path was fixed in v0.21.2 (#98); this closes the same defect on the `ui` path, which is the one a settings menu goes through. **Assets already broken this way are repairable in place** with `blueprint set_variable_type` and the same `enum:<Name>` string — recreating the variable does not help, because the old grammar rebuilt it exactly as wrong. (#115)
- **`enum:` accepts a full object path.** `enum:/Script/UMG.ESlateVisibility` and `enum:/Game/Enums/E_Ammo` never resolved on any action, on any version: lookup went through `FindFirstObject`, which converts its entire argument into a single `FName`, so a path could only ever miss. Paths now resolve through `FindObject`/`LoadObject`, short names still take the fast `FindFirstObject` route, and a name written without the conventional `E` prefix (`enum:SlateVisibility`) is retried with it.
- **The `risk` namespace mined no repositories on almost every project.** The git roots were a hardcoded list of six nested-plugin paths. The project root was never probed and no setting could add it, so for anyone whose project root *is* the git repository — the ordinary layout — every root was skipped, the skip was logged at `Verbose`, and the run still stamped its "up to date" marker, leaving the staleness check unable to ever re-fire. `risk_query` returned empty permanently with nothing in the response to say why. Roots are now discovered at runtime: the project root when it holds a `.git` entry, plus each immediate `Plugins/<Name>` that holds one, with `.git` probed as a file *or* a directory so submodule and worktree checkouts resolve. A nested plugin repository is kept even when an ancestor repository is also mined, because git reports a nested repository as untracked and the two file sets are therefore disjoint. Two new settings under **Editor Preferences → Plugins → Monolith Reflection Intel → Risk**: `GitRepoRoots`, where empty auto-resolves and a non-empty list replaces the auto set outright, and `bProbeAncestorsForGitRoot`, off by default, for a project checked out inside a larger repository. (#119)
- **Hotspot scores were `0` for every file.** `git log` reports paths relative to the repository it runs in, so a nested plugin repository produced churn keyed `Source/…` while the complexity side keyed `Plugins/<Name>/Source/…`. The two maps never met: every file landed as two half-populated rows and `score = normalised_churn × normalised_complexity` evaluated to `0` for both of them. Churn paths are now rebased into the project-relative space before tallying — prefixed for a repository under the project, offset-stripped for one containing it, with rows outside the project subtree dropped, and left alone for an unrelated absolute root. This changes the stored row shape, so the risk indexer version goes `1 → 2` and existing risk tables rebuild once on the first query after upgrading.
- **Editing a risk setting did nothing until the editor was restarted.** Staleness detection watched only the indexer code version, and the per-session bootstrap latch is set before mining runs, so a changed window size, file cap, noise filter or repository list was simply ignored by data already on disk. A configuration fingerprint over the resolved roots and those three settings is now stamped as a second row in the existing `monolith_ri_meta` table, which needs no schema change because `subsystem` is already a text primary key. One shared function computes it for both the writer and the reader, so the two cannot drift, and any Risk-category edit clears the bootstrap latch and the retry throttle together so the next `risk_query` re-mines. A missing fingerprint counts as a mismatch, so an existing database re-mines once after upgrading.
- **Blueprint component reads returned the C++ parent's defaults presented as the Blueprint's own values.** `get_component_details` and `get_components` read the class-default object of `ParentClass` — the native class template, i.e. Epic's constructor defaults — while `set_component_property` wrote the Blueprint's `GeneratedClass` CDO. Read and write were aimed at different objects, so on any Character Blueprint the read actions reported the engine's values rather than the project's: a capsule half-height overridden to 96 read back 88, and the blast radius covered transforms, collision, movement settings and mesh/anim-class wiring, not just the two null fields in the report. Both read paths now use the Blueprint's own CDO, through one shared resolver that the write path uses as well — there were five independent component resolvers in the module and three of them disagreed about which class-default object to read. Alongside the values, `get_component_details` reports `source` — one of `scs`, `cdo_native`, `ich_override`, `inherited_scs`, `parent_cdo_fallback` — plus `resolved_component` and a `note` when the value carries a caveat, and `get_components` gains `inherited_components[]` for components declared on a parent Blueprint's construction script, which sit on no class-default object and were invisible to every read action before. `parent_cdo_fallback` covers the case a naive fix would have broken: a Blueprint with no compiled generated class still lists its components, with a note saying the values are native defaults pending a recompile, instead of answering "component not found". (#116)
- **The `Mesh` alias resolved to the capsule on Character Blueprints.** The mechanism was not a name mixup: the alias table carried no class of its own, so the class fallback matched the *caller's* required class — and a read action that must serve any component has to pass `UActorComponent`, which the first component on the class-default object satisfies. Aliases now carry their own target class (`Mesh`/`SkeletalMesh`, `StaticMesh`, `CharacterMovement`/`Movement`, `Capsule`/`CapsuleComponent`, `Root`/`RootComponent`), an exact component name always beats an alias, and an alias matching more than one component of its class reports the ambiguity with the candidate names rather than silently picking the first. `CapsuleComponent` resolves, and `get_inherited_component_override` accepts `asset_path` as an alias for `bp_path`. (#116)
- **`set_component_property` can now write a component inherited from a parent Blueprint.** Those components live on no class-default object, so the write used to fail with "component not found"; it now creates an Inheritable Component Handler override on the child Blueprint and leaves the parent's template alone. The write happens before the compile, because the compiler prunes any override record identical to its archetype — creating the override and compiling first discards it silently. The response reports `persisted`, which is `false` when the written value equalled the inherited default and the record was pruned, with a `note` saying so. Four guards accompany it: the component key is validated against the Blueprint's parent chain the way the editor's own call sites do; a Blueprint with no compiled generated class is rejected up front, because the engine accessor does a `CastChecked` and would otherwise take the editor down from an ordinary call; `[Kismet] bEnableInheritableComponents=false` is reported as the configuration answer it is; and the value is re-read after the compile rather than through a pointer the compile may have moved into the transient package. (#102)
- **`project search` no longer reports query errors as zero results.** The result loop was `while (Step() == Row)`, which treats a failed step as the end of the rows — so malformed FTS5 syntax came back as a successful, empty result set, indistinguishable from a genuine no-match. `FMonolithIndexDatabase::FullTextSearch` gains a four-argument overload returning `EMonolithProjectSearchStatus` (`Succeeded` / `InvalidQuery` / `InternalError`), and `project search` maps it to a JSON-RPC code: a caller's malformed query is `-32602`, a storage or schema failure is `-32603`. The `Create` and `SetBindingValueByIndex` return values are now checked as well — a prepare failure previously yielded zero results silently. (#113)
- **`project search` no longer builds its `LIMIT` clause by string formatting.** The limit is bound as a statement parameter alongside the query.
- **A column filter naming a column no index exposes now says which one.** The error names the offending column and lists the valid ones for both FTS tables — `no such column 'node_nme'` rather than a bare SQLite diagnostic. Column filters that are valid for one table remain valid: a filter is only rejected when **both** tables refuse it, so `node_name:Branch` and `asset_name:BP_Enemy` keep working while `asset_name:X OR node_name:Y` (which neither table can answer) is refused.
- **Corrected the documented `NEAR` syntax for `project search`.** The `unreal-project-search` skill documented `BP_Enemy NEAR/3 Health`, which is not FTS5 syntax and always errored. The working form is `NEAR(BP_Enemy Health, 3)`.
- **Corrected the documented scope of `project search`.** The action description and skill claimed coverage of "variables and parameters". The full-text index covers asset `name`/`class`/`description`/`package_path`/`module_name` and graph node `name`/`class`/`type`; variables and parameters are structured rows, reached via `get_asset_details`. The skill now also documents the `limit` parameter, per-column filters, and which column-filter combinations the two FTS tables can and cannot answer.
- **`monolith_discover` pagination no longer overflows into an empty page with a negative `next_offset`.** `SliceEnd` was formed as `SliceStart + Limit` in `int32` before being clamped, and a huge `limit` is a well-formed request — `TryGetNumberField` range-checks against `TNumericLimits<int32>`, so it accepts `2147483647` verbatim while rejecting anything larger. With `offset: 1` the sum wrapped negative on MSVC, the clamp collapsed the slice, and the caller got an empty `actions` array alongside a negative `next_offset`. The `Min()` is now taken first so every intermediate stays in range, and `next_offset` emits `SliceEnd`, which is value-identical and structurally overflow-free. (#112)
- **Stopping or restarting the Monolith server no longer takes every other plugin's HTTP routes down with it.** `Stop()`, `Monolith.Restart` and the bind-retry loop all called `FHttpServerModule::StopAllListeners()`, which is process-wide: it stops every listener in the editor, so Web Remote Control, PerfCounters, ExternalRpcRegistry and any plugin holding a route lost service because Monolith was restarting. Teardown now unbinds only the six routes Monolith owns and leaves the shared listener alone. Removing that call is also what makes `Monolith.Restart` able to recover a wedged listener at all: `StopAllListeners` clears the module's listeners-enabled flag, and with the flag down `GetHttpRouter` hands back the *dead* listener instead of evicting it, so the old restart could never rebuild a listener that had failed to bind. **One consequence worth knowing:** the listener now survives `Stop()`, so the port stays held until the editor exits and a TCP probe of port 9316 is no longer a valid liveness check for Monolith — use `monolith_status`. (#114)
- **Closing the editor during a project index could crash on shutdown.** `FIndexingTask::Run` posted four fire-and-forget game-thread callbacks — the database-open failure exit, both per-batch progress broadcasts, and the final completion — each capturing the task by pointer. Shutdown joins the worker thread, but joining does not drain the game thread's task queue, so an already-queued callback ran after the task object had been destroyed and read its counters and owner through a dangling pointer. All four now capture a weak pointer to the subsystem and a copy of the values they report, taken before the callback is constructed. The progress broadcasts fire once per batch rather than once per run, so they were the likeliest of the four to be in flight when the editor closed. The same pattern is applied to the C++ source indexer's completion callback, which reopened the source database through a raw pointer to a subsystem that may already have been torn down.
- **A failed project index left the asset index silently stale for the rest of the session.** The four live Asset Registry callbacks are detached for the duration of an index run and were re-armed only on success. After a failed or cancelled run they stayed detached: the subsystem reported itself active, `project_query` kept answering, and every subsequent asset change was dropped until a successful reindex or an editor restart. They are now re-armed on every outcome. `UnregisterLiveCallbacks` also resets its delegate handles — a retained handle read as "still armed" and would have blocked every later re-arm — and registration is a no-op while indexing, while the database is closed, or when a handle is already bound, so it cannot double-bind and leave a first copy permanently attached.
- **A reindex that never started no longer reports success.** `monolith.reindex`, `source.trigger_reindex` and `source.trigger_project_reindex` answered unconditionally, so an agent got `reindex_started` for a request the subsystem had already refused. All four entry points now return a real result and the actions report it: `monolith.reindex` returns `status: "reindex_not_started"` with the reason, and the two `source` actions return an error naming the cause. `monolith.reindex` reaches the index subsystem only by reflection, so it checks the reflected return property before reading it and falls back to the old response shape rather than reporting "not started" for work that did start. The Live Coding auto-reindex hook likewise stamps its five-second cooldown only when a run actually began, instead of suppressing the next five seconds of auto-kicks on behalf of a reindex that never ran.
- **An indexer that failed to start refused every request afterwards until the editor was restarted.** Two exits from the C++ source indexer returned without broadcasting completion — a failed worker-thread creation, and a failed database open. The owning subsystem clears its in-progress flag from that broadcast, so either failure latched the flag, and every later reindex was rejected with "Indexing already in progress" for the rest of the session. Every exit now routes through a single completion path that clears the flag and broadcasts, and an automation test covers the database-open case. The project indexer's equivalent thread-creation failure unwinds the same way.
- **Editor startup no longer stalls on the port probe.** Monolith verifies its bind by connecting to `127.0.0.1` on the configured port. The probe used a blocking socket, so on Windows an attempt against a closed port sat on the game thread for roughly two seconds before the OS gave up — on the startup path, once per retry. It is now non-blocking. The socket's connection state is what decides the verdict, not `Connect`'s return value: on a non-blocking socket that call reports success for a connection merely *in progress*, and since the probe is the only success check in the bind loop, trusting it would have Monolith logging that it was listening having bound nothing. The blocking path remains as a fallback if the platform refuses the mode change.
- **An interrupted full index no longer throws away the work it had already committed.** A full index used to wipe the database at the start and write its completion marker only at the very end, so any crash, kill or power loss left no marker — and the next launch began another full index, which immediately erased every batch the previous run had finished. On a large project one crash cost hours; repeated crashes meant the index never completed at all. (#117, #120)

  The index now checkpoints. Schema v3 adds two columns to `assets` (`deep_indexed_hash`, `deep_index_attempts`) and an `in_progress` marker to `meta`; migration is automatic, additive, and leaves existing rows untouched. An interrupted run resumes on the next launch: assets already deep-indexed at their current content hash are not redone, and assets whose content changed are re-indexed normally. The hash checkpoint is written in the same transaction as the data it vouches for, so a rollback can never leave a checkpoint pointing at rows that are not there.

  A resume necessarily re-queues the asset that killed the editor, so the index counts attempts — and that counter commits in its own transaction *before* the batch runs, the exact opposite of the hash checkpoint, because a counter that rolled back with the crash could never fire. An asset that twice began deep indexing without finishing is dropped from the queue rather than crashing the editor a third time. That drop is deliberate and never silent — every skipped path is logged at Error, recorded in the database, and reported by `project get_stats` as `skipped_assets`. Because attempts are counted per batch, the rest of that batch is dropped with it. Recover with `Monolith.StartIndex force` (or `monolith_reindex force=true`), which wipes and clears the counters.

  `bDeferFirstTimeIndex` is honoured on the resume path. It is the escape hatch for exactly the crash class an interrupted index is evidence of, so a resume waits for `Monolith.StartIndex` like a first-time index does; the checkpoints are durable, so waiting costs nothing.
- **A project with fewer than 500 assets no longer re-indexes from scratch on every launch.** The completion marker was gated on an asset count meant to detect a half-loaded Asset Registry, so small projects never recorded a completed index. The marker is now gated on structural success — cancellation and transaction failures — and not on per-asset errors, which occur on any real project (a class from a disabled plugin, a redirector stub, an animation asset that fails to load) and would otherwise block the index forever from the opposite direction.
- **The animation indexing pass commits per batch instead of once at the end**, so an interruption costs one batch rather than every animation asset indexed since the pass began. The pass also releases the assets it loads (assets already resident before the pass are left alone), which the per-batch garbage collection could not previously act on.
- **A failed database reset aborts the index instead of being ignored.** `ResetDatabase()`'s return value was discarded, so a failed reset left the old rows in place and the run then fought UNIQUE constraints on every insert. It also now restates `schema_version` after recreating the tables — the reset drops `meta`, which previously left the database reporting no schema version until the next editor launch, silently disabling incremental indexing for the session.

### Changed

- **`animation set_anim_graph_node_property` reports where the write landed.** The response gains `resolved_graph_path`, the root-to-leaf graph path of the node that was actually written (e.g. `Standing Stance/Standing States/Stop/Stop States/Plant Left Foot`), alongside the existing `old_value` / `new_value`. It is emitted for scoped and unscoped calls alike, so a caller can assert the target of a write rather than inferring it from `old_value`.
...
- **Pin-type strings are parsed strictly by the actions that create pins.** The pin-type grammar existed as two hand-maintained copies — one in `MonolithBlueprint`, one in `MonolithUI` — which is how they drifted into disagreeing about enums in the first place; there is now one shared implementation. With it, the actions that create variables, function parameters and event-dispatcher parameters reject a type they cannot resolve instead of quietly substituting something else. Previously a typo'd `type` in `blueprint set_function_params` produced a `bool` pin, and an unresolvable `enum:Whatever` or `struct:Whatever` in `blueprint add_variable` produced a variable with no type object at all; both now return an error naming the token and the reason, including for a container's value type (`map:string:object:Bogus`), which no previous check looked at. `set_function_params` and `set_event_dispatcher_params` validate every requested type *before* creating or removing a single pin, so a rejected call leaves the existing signature untouched rather than half-rewritten.
- **`blueprint get_inherited_component_override` reports a wider set of `source` values.** It previously returned `cdo_native`, `ich`, or `scs`, and its `ich` branch could never actually fire — it compared Inheritable Component Handler record names against a class-default-object component's name, which are different objects. The value set is now `scs`, `cdo_native`, `ich_override`, `inherited_scs`, `parent_cdo_fallback`, shared with `get_component_details` and `set_component_property`. Callers matching on the literal `"ich"` need updating; `cdo_native` and `scs` are unchanged.
- **`project search` validates its parameters before touching the index.** `query` must be a string, is trimmed, must be non-empty, and is capped at 4096 characters; `limit` must be an integer and is clamped to 1-1000. Each rejection is `-32602`. Previously a non-string `query` or a non-integer `limit` reached `GetStringField`/`GetIntegerField` unchecked, `limit` was unbounded in both directions, and `limit: 0` silently returned nothing. The query cap bounds the parse work a single MCP call can hand the game thread, where `FullTextSearch` runs.
- **The Project Settings re-index button is greyed out when the index database is closed.** Previously it was enabled whenever an index was not running, including when the database had failed to open — pressing it did nothing and said nothing.
- **`Monolith.StartIndex` resumes rather than wipes.** Use `Monolith.StartIndex force` for the old behaviour. `monolith_reindex force=true` is unchanged and still performs a full wipe and rebuild.
