		...
		if (LevelIndexer && LevelIndexer->IsValid())
		{
			...
			if (LevelRaw)
			{
				...
				for (int32 BatchStart = 0; BatchStart < TotalWorlds && !CheckCancellation(); BatchStart += LevelBatchSize)
				{
					...
					// The level pass runs batched rather than through RunSentinelInTransaction
					// (one transaction per batch, not one for the whole pass) so each batch's
					// worlds can be torn down and collected before the next loads. It still owes
					// the completion gate the same structural-failure signal that lambda gives
					// every other indexer, so the transaction results are checked here too.
					RunOnGameThreadBlocking(
						[DB, LevelIndexerCopy, LevelRaw, &bTransactionFailure, BatchSlice = MoveTemp(BatchSlice)]()
					{
						if (!DB->BeginTransaction())
						{
							bTransactionFailure = true;
							UE_LOG(LogMonolithIndex, Error, TEXT("Index transaction failure (level batch begin) - this run will not be marked complete"));
							return;
						}
						...
						{
							bTransactionFailure = true;
							UE_LOG(LogMonolithIndex, Error, TEXT("Index transaction failure (level batch commit) - this run will not be marked complete"));
						}
					});
